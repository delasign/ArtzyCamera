...
//  ArtzyCamera
//
//  Created by Oscar De la Hera Gomez on 12/24/18.
...
vertex out_vertex_t blue_mask_vertex(custom_vertex_t in [[stage_in]],
...
fragment half4 blue_mask_fragment(out_vertex_t in [[stage_in]],
...
vertex out_vertex_t blue_combine_vertex(custom_vertex_t in [[stage_in]],
                                       constant Uniforms& uniforms [[ buffer(0) ]])
{
...
fragment half4 blue_combine_fragment(out_vertex_t vert [[stage_in]],
                                    texture2d<float, access::sample> colorSampler [[texture(0)]],
                                    ...
                                    constant Uniforms& uniforms [[ buffer(0) ]])
{
    ...
    float r = 0.1411764706; // 0.56
    float g = 0.4; // 0.84
    float b = 0.6470588235; // 0.95
...
vertex out_vertex_t blue_blur_vertex(custom_vertex_t in [[stage_in]],
                                    constant Uniforms& uniforms [[ buffer(0) ]])
{
    float test = uniforms.symbolAsCalledInMetal;
    out_vertex_t out;
...
fragment half4 blue_blur_fragment_h(out_vertex_t vert [[stage_in]],
...
fragment half4 blue_blur_fragment_v(out_vertex_t vert [[stage_in]],
                                   texture2d<float, access::sample> maskSampler [[texture(0)]],
                                   constant Uniforms& uniforms [[ buffer(0) ]])
{
    float test = uniforms.symbolAsCalledInMetal;
