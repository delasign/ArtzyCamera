                                ...
                                constant Uniforms& uniforms [[ buffer(0) ]])
{
    ...
    out.modelViewProjectionTransformColumn = float4(0,0,0,0);
    return out;
};
                                ...
                                constant Uniforms& uniforms [[ buffer(0) ]])
{
    ...
    out.uv = float2( (in.position.x + 1.0) * 0.5, 1.0 - (in.position.y + 1.0) * 0.5 );
    out.modelViewProjectionTransformColumn = float4(0,0,0,0);
    //    out.modelViewProjectionTransformColumn = scn_node.modelViewProjectionTransform[3];
    return out;
};
                               ...
                               constant Uniforms& uniforms [[ buffer(0) ]])
{
    ...
    for (int i=1; i<5; i++) {
        FragmentR += maskSampler.sample( s, ( vert.uv + float2(offset[i], 0.0)/bufferSize ) ).r * weight[i] * uniforms.symbolAsCalledInMetal;
        FragmentR += maskSampler.sample( s, ( vert.uv - float2(offset[i], 0.0)/bufferSize ) ).r * weight[i] * uniforms.symbolAsCalledInMetal;
    }
                               ...
                               constant Uniforms& uniforms [[ buffer(0) ]])
{
    ...
    for (int i=1; i<5; i++) {
        FragmentR += maskSampler.sample( s, ( vert.uv + float2(0.0, offset[i])/bufferSize ) ).r * weight[i] * uniforms.symbolAsCalledInMetal;;
        FragmentR += maskSampler.sample( s, ( vert.uv - float2(0.0, offset[i])/bufferSize ) ).r * weight[i] * uniforms.symbolAsCalledInMetal;;
    }
